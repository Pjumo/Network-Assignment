...
% PL_cost1 = 46.3 + 33.9 * log10(fq) - 13.82*log10(tx_h) - 3.2 * (log10(11.75 * rx_h))^2 - 4.97 + (44.9 - 6.55 * log10(tx_h)) * log10(d1) + 3;
PL_sui1 = 20*log10(4*pi*100*fq)-147.55+10*(3.6-0.005*tx_h+20/tx_h)*log10(d1/100)+6*log10(fq/2000)-20*log10(rx_h/2);
% PL_cost2 = 46.3 + 33.9 * log10(fq) - 13.82*log10(tx_h) - 3.2 * (log10(11.75 * rx_h))^2 - 4.97 + (44.9 - 6.55 * log10(tx_h)) * log10(d2) + 3;
PL_sui2 = 20*log10(4*pi*100*fq)-147.55+10*(3.6-0.005*tx_h+20/tx_h)*log10(d2/100)+6*log10(fq/2000)-20*log10(rx_h/2);
% PL_cost3 = 46.3 + 33.9 * log10(fq) - 13.82*log10(tx_h) - 3.2 * (log10(11.75 * rx_h))^2 - 4.97 + (44.9 - 6.55 * log10(tx_h)) * log10(d3) + 3;
PL_sui3 = 20*log10(4*pi*100*fq)-147.55+10*(3.6-0.005*tx_h+20/tx_h)*log10(d3/100)+6*log10(fq/2000)-20*log10(rx_h/2);
% PL_cost4 = 46.3 + 33.9 * log10(fq) - 13.82*log10(tx_h) - 3.2 * (log10(11.75 * rx_h))^2 - 4.97 + (44.9 - 6.55 * log10(tx_h)) * log10(d4) + 3;
PL_sui4 = 20*log10(4*pi*100*fq)-147.55+10*(3.6-0.005*tx_h+20/tx_h)*log10(d4/100)+6*log10(fq/2000)-20*log10(rx_h/2);

PL_sui1(:, :, 2) = PL_sui1(:, :, 2) + 6.021;
PL_sui2(:, :, 2) = PL_sui2(:, :, 2) + 6.021;
PL_sui3(:, :, 3) = PL_sui3(:, :, 3) + 1.2498;
PL_sui4(:, :, 3) = PL_sui4(:, :, 3) + 1.2498;
...
PL_else3=zeros(101, 101, 2);
PL_else4=zeros(101, 101, 2);
for c = 1:1:101
    for r = 1:1:101
        j=1;
        k=1;
        ...
        if j==1
            ...
            PL_else3(c, r, 2) = PL_max3(c, r);
        end
        if k==1
            PL_else4(c, r, 1) = PL_max4(c, r);
            PL_else4(c, r, 2) = PL_max4(c, r);
        end
        if j==2
            PL_else3(c, r, 2) = PL_max3(c, r);
        end
...
for c = 1:1:101
    for r = 1:1:101
        ...
        for i=1:2
            ...
            itf3 = itf3 + 10^(signal_else3(c, r, i)/10);
            itf4 = itf4 + 10^(signal_else4(c, r, i)/10);
        end
        SINR_w1(c, r) = 10^(signal_max1(c, r)/10)/itf1 + N;
        SINR_w2(c, r) = 10^(signal_max2(c, r)/10)/itf2 + N;
        SINR_w3(c, r) = 10^(signal_max3(c, r)/10)/itf3 + N;
        SINR_w4(c, r) = 10^(signal_max4(c, r)/10)/itf4 + N;
    end
end
